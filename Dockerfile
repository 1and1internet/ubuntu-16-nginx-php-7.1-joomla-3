...
RUN \
    ...
    JOOMLA_DOWNLOAD=https://downloads.joomla.org$(curl -fsL https://downloads.joomla.org/latest | grep -Eo '/cms/joomla3/.*-stable-full_package-tar-gz\?format=gz' | sort -nr | uniq | head -1) && \
